...
class ProgressTracker:
    ...
    def get_user_id(self) -> str:
        """Obtiene o crea un ID de usuario único"""
    ...
    def get_overall_progress(self) -> float:
        """Calcula el progreso general del usuario"""
        total_modules = len(self.progress_data["module_completion"])
        if total_modules == 0:
            return 0.0
        
        completed_count = 0
        for module_id, tasks in self.progress_data["module_completion"].items():
            if len(tasks) > 0:  # Si tiene alguna tarea completada
                completed_count += 1
    ...
    def add_quiz_score(self, module_id: str, score: float, max_score: float):
        ...
        if user_id not in self.progress_data["quiz_scores"]:
            self.progress_data["quiz_scores"][user_id] = {}
        
        if module_id not in self.progress_data["quiz_scores"][user_id]:
            self.progress_data["quiz_scores"][user_id][module_id] = []
        ...
        self.progress_data["quiz_scores"][user_id][module_id].append(quiz_result)
        self._save_progress_data()
    ...
    def get_quiz_scores(self, module_id: str) -> List[Dict]:
        ...
        if (user_id in self.progress_data["quiz_scores"] and 
            module_id in self.progress_data["quiz_scores"][user_id]):
            return self.progress_data["quiz_scores"][user_id][module_id]
